...
function dashboardTerrain(){
 ...
 const priority=prospects.filter(p=>active(p)&&movement6mInfo(p).score>=65).length;
 const recent=prospects.filter(p=>active(p)&&signalInfo(p).tags.some(t=>t.key==="new")).length;
 const addressReady=prospects.filter(p=>active(p)&&p.address&&p.postalCode&&p.city).length;
 const addressMissing=prospects.filter(p=>active(p)&&(!p.address||!p.postalCode||!p.city)).length;
 const follow=prospects.filter(p=>p.nextFollow&&new Date(p.nextFollow+"T23:59:59").getTime()<nowMs&&active(p)).length;
 const changes=prospects.filter(p=>{const t=new Date(p.updatedAt||p.createdAt||0).getTime();return active(p)&&Number.isFinite(t)&&nowMs-t<=recentMs&&(signalInfo(p).tags.some(x=>["priceDown","multi"].includes(x.key))||movement6mInfo(p).tags.some(x=>["repeatedAppearance","repeatedPrice"].includes(x.key)));}).length;
 const weekNew=prospects.filter(p=>{const t=new Date(p.detectionDate||p.createdAt||0).getTime();return Number.isFinite(t)&&nowMs-t<=weekMs;}).length;
 const weekFollow=prospects.filter(p=>{const t=new Date(p.updatedAt||0).getTime();return Number.isFinite(t)&&nowMs-t<=weekMs&&p.history?.some(h=>String(h.type||"").toLowerCase().includes("relance"));}).length;
 const readyVisit=Math.min(10,prospects.filter(p=>active(p)&&p.address&&p.postalCode&&p.city&&p.sourceUrl).length);
 const byCity={};prospects.filter(active).forEach(p=>{const city=String(p.city||"").trim();if(city)byCity[city]=(byCity[city]||0)+1});
 const sectors=Object.entries(byCity).sort((a,b)=>b[1]-a[1]).slice(0,5);
 $("dashboardTerrain").innerHTML='<div class="dashboard-head"><div><h2>🎯 À faire maintenant</h2><p>Vue opérationnelle pour organiser ta prospection terrain.</p></div><span class="dashboard-badge">'+priority+' priorité'+(priority>1?'s':'')+' terrain</span></div><div class="dashboard-actions-grid"><button class="dashboard-action action-hot" data-dashboard-action="priority"><strong>🔥 '+priority+'</strong><span>Priorités terrain</span><small>Indice ≥ 65</small></button><button class="dashboard-action" data-dashboard-action="recent"><strong>🆕 '+recent+'</strong><span>Nouveaux récents</span><small>Détectés ≤ 7 jours</small></button><button class="dashboard-action" data-dashboard-action="address"><strong>📍 '+addressMissing+'</strong><span>Adresses à compléter</span><small>Informations manquantes</small></button><button class="dashboard-action" data-dashboard-action="follow"><strong>📞 '+follow+'</strong><span>Relances en retard</span><small>À traiter maintenant</small></button><button class="dashboard-action" data-dashboard-action="changes"><strong>🔄 '+changes+'</strong><span>Changements récents</span><small>Prix / apparitions / sources</small></button><button class="dashboard-action" data-dashboard-action="tour"><strong>🚗 '+readyVisit+'</strong><span>Biens prêts terrain</span><small>Adresse + source publique</small></button></div><div class="dashboard-lower"><div class="dashboard-box"><h3>🗺️ Secteurs actifs</h3>'+(sectors.length?sectors.map(([city,n])=>'<button class="sector-row" data-dashboard-city="'+esc(city)+'"><span>'+esc(city)+'</span><strong>'+n+'</strong></button>').join(""):'<div class="meta">Aucune commune renseignée.</div>')+'</div><div class="dashboard-box"><h3>📊 Activité récente</h3><div class="activity-row"><span>Nouveaux cette semaine</span><strong>'+weekNew+'</strong></div><div class="activity-row"><span>Fiches avec adresse exploitable</span><strong>'+addressReady+'</strong></div><div class="activity-row"><span>Relances enregistrées cette semaine</span><strong>'+weekFollow+'</strong></div><div class="activity-row"><span>Total prospects actifs</span><strong>'+prospects.filter(active).length+'</strong></div></div></div><div class="dashboard-note">Les priorités sont des indicateurs internes basés sur les données présentes dans JML Prospection. Elles servent à organiser ton travail et ne prédisent pas qu'un propriétaire va vendre.</div>';
}
...
$("dashboardTerrain").onclick=e=>{
 ...
 if(city){$("city").value=city.dataset.dashboardCity;render();$("list").scrollIntoView({behavior:"smooth",block:"start"});return}
 if(!b)return;
 const action=b.dataset.dashboardAction;
 if(action==="priority"){$("movementFilter").value="high";render();$("list").scrollIntoView({behavior:"smooth",block:"start"});}
 else if(action==="recent"){$("signalFilter").value="new";render();$("list").scrollIntoView({behavior:"smooth",block:"start"});}
 else if(action==="follow"){$("signalFilter").value="follow";render();$("list").scrollIntoView({behavior:"smooth",block:"start"});}
 else if(action==="changes"){$("signalFilter").value="priceDown";render();$("list").scrollIntoView({behavior:"smooth",block:"start"});}
 else if(action==="address"){$("q").value="";$("city").value="";$("signalFilter").value="incomplete";render();$("list").scrollIntoView({behavior:"smooth",block:"start"});}
