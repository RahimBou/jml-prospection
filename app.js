...
const KEY="jml_prospection_v1";let prospects=load(),pendingImport=[];let prospectPage=1;const DEFAULT_PROSPECT_PAGE_SIZE=8;const $=id=>document.getElementById(id);
...
function dashboardTerrain(){
 ...
 const changes=prospects.filter(p=>{const t=new Date(p.updatedAt||p.createdAt||0).getTime();return active(p)&&Number.isFinite(t)&&nowMs-t<=recentMs&&signalInfo(p).tags.some(x=>x.key==="priceDown");}).length;
 ...
 const weekFollow=prospects.reduce((n,p)=>n+(p.history||[]).filter(h=>{const t=new Date(h.date||0).getTime();const text=String((h.type||"")+" "+(h.text||"")).toLowerCase();return Number.isFinite(t)&&nowMs-t<=weekMs&&text.includes("relance");}).length,0);
...
if($("prospectPagination"))$("prospectPagination").onclick=e=>{const b=e.target.closest("[data-page]");if(!b)return;const total=Math.max(1,Math.ceil(prospects.length/(Number($("prospectPageSize").value)||DEFAULT_PROSPECT_PAGE_SIZE)));const target=b.dataset.page==="prev"?prospectPage-1:b.dataset.page==="next"?prospectPage+1:Number(b.dataset.page);if(target>=1&&target<=total){prospectPage=target;render();$("prospectsPanel").scrollIntoView({behavior:"smooth",block:"start"})}};
