...
async function ctFindAddress(index){
  ...
  try{
    ...
    box.innerHTML=rows.length?'<div class="ct-address-title">Adresses candidates · concordance technique</div>'+rows.map((c)=>'<div class="ct-address-candidate"><strong>'+apiEsc(c.address)+'</strong><span>'+apiEsc([c.postalCode,c.city].filter(Boolean).join(" "))+' · '+Math.round(c.distanceMeters||0)+' m · <b>'+Math.round(c.score||0)+'/100</b> · '+apiEsc(c.confidence==="forte"?"concordance forte":c.confidence==="moyenne"?"concordance moyenne":"concordance faible")+'</span><small>'+apiEsc((c.reasons||[]).join(" · "))+'</small>'+(c.dpe?'<small>DPE candidat : '+apiEsc(c.dpe.dpe||"—")+' · '+(c.dpe.area||"—")+' m² · '+(c.dpe.rooms||"—")+' pièce(s)'+(c.matchable?" · candidat exploitable":" · candidat écarté")+'</small>':"<small>Aucun DPE public suffisamment rapproché</small>")+'</div>').join("")+'<div class="meta">Adresse non confirmée : le résultat sert à orienter le rapprochement public, pas à identifier un propriétaire.</div>':'<div class="meta">Aucune adresse candidate suffisamment documentée.</div>';
