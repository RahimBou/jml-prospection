...
function futureRadarRender(){
  ...
  box.innerHTML=futureRadarCandidates.slice(0,100).map((p,i)=>{
    ...
    const count=p.selectedMutationCount!==undefined?'<span>Mutations retenues : '+p.selectedMutationCount+(p.matchedMutationCount>p.selectedMutationCount?' / '+p.matchedMutationCount+' à l’adresse':'')+'</span>':"";
