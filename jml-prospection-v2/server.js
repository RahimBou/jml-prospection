...
async function api(req, res, url) {
  ...
  if (url.pathname === "/api/sources") {
    return send(res, 200, {
      ...
      sources: [
        ...
        { id: "dvf", label: "DVF open data", status: "planned" },
        { id: "dpe", label: "DPE open data", status: "planned" }
      ]
    });
  }
  ...
  if (url.pathname === "/api/marche") {
    const params = Object.fromEntries(url.searchParams.entries());
    const result = await buildMarketSnapshot(params);
    return send(res, 200, result);
  }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    console.error(error);
    send(res, 500, { error: "Erreur serveur", message: error.message });
  }
});
...
server.listen(PORT, "0.0.0.0", () => {
  console.log("JML Prospection V2 listening on 0.0.0.0:" + PORT);
});
